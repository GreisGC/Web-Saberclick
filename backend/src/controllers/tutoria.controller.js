...
const getAllTutoria = async (req, res, next) => {
    try {
        const allTutoria = await pool.query("SELECT * FROM tutoria WHERE estado = 'Habilitado'");
...
const getTutoria = async (req, res, next) => {
    try {
        const { id } = req.params;
        const result = await pool.query("SELECT * FROM tutoria WHERE id_tutoria = $1 AND estado = 'Habilitado'", [id]);
...
const createTutoria = async (req, res, next) => {
  ...
  try {
    const { costo, nombre_tutoria, descripcion, nombre_institucion } = req.body;
    ...
    if (institucionResult.rows.length === 0) {
      ...
      return res.status(404).json({ message: "Institución no encontrada o deshabilitada" });
    }
    ...
    if (tutoriaExistente.rows.length > 0) {
      await client.query('ROLLBACK');
      return res.status(400).json({ message: "Ya existe una tutoría con ese nombre" });
    }
...
const deleteTutoria = async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "Tutoría y todas sus entidades relacionadas deshabilitadas correctamente",
      tutoria: tutoriaResult.rows[0],
    });
