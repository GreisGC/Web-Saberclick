...
const ScheduleBlock = ({ data, onClick }) => (
	<Paper elevation={3} sx={{ p: 2, mb: 3 }}>
		<Grid container spacing={2}>
			...
			<Grid item xs={12} sx={{ textAlign: "right", mt: 2 }}>
				{/* <InscripcionParalelo id_paralelo={3} id_tutoria={1} /> */}
				<Button
					variant="contained"
					onClick={() => {
						onClick(data.id_paralelo, data.id_tutoria);
						console.log(
							"Inscripción:",
							data.id_paralelo,
							data.id_tutoria
...
export default function ParaleloHome() {
	...
	const { id_paraleloI, id_tutoriaI, setId_paraleloII, setId_tutoriaII } =
		useContext(SesionContext);

	const [paralelo2, setParalelo] = useState(null);
	...
	return (
		<Box sx={{ mt: 3 }}>
			...
			{!paralelo2 ? (
				...
				<ScheduleBlock
					data={paralelo2}
					onClick={(id_paralelo, id_tutoria) => {
						setId_paraleloII(id_paralelo);
						setId_tutoriaII(id_tutoria);
						navigate("/inscripcion/evaluacion");
					}}
				/>
			)}
		</Box>
